...
| Rule | Detail |
|------|--------|
| Context handshake | At session start, call `GET /v1/scry/context` and include `skill_generation` for packaged skills. Honor `should_update_skill`, check `client_skill_generation`, and use typed search, `scry.search_federated(...)`, or source-native `scry.search_*` helpers for lexical discovery. The `lexical_search.*` status fields describe the old shared BM25 diagnostic path; they do not define the lexical-serving contract. If the response shows `client_skill_generation: null` while you're using packaged skills, or if local instructions still mention legacy ExoPriors hostnames or legacy console routes, run `npx skills update scry --yes` before more debugging. |
